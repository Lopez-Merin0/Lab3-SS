...
function getFileEmoji(name) {
  ...
  const map = { pdf: '📄', zip: '🗜', exe: '⚙', doc: '📝', docx: '📝', js: '📜', py: '🐍', sh: '💻', jpg: '🖼', png: '🖼', mp4: '🎬', mp3: '🎵' };
  return map[ext] || '📁';
}
...
function handleFileSelect(file) {
  ...
  // Si pasa las validaciones, continuar normal
  selectedFile = file;
...
// MOCK — replace with real fetch('/api/scan', { method:'POST', body: formData })
async function mockScanAPI(file) {
  await sleep(300);
  // Simulate: files with "test" in name are "infected"
